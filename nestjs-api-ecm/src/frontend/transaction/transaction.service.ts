...
// Định nghĩa class TransactionService và inject repository của TransactionEntity
@Injectable()
export class TransactionService {
    ...
    // Inject ProductService, OrderService và ServiceCore vào constructor
    constructor(
    ...
    // Lấy danh sách giao dịch dựa trên phân trang và bộ lọc
    async getListsTransaction(paging: any, filters: any) {
        ...
        // Sử dụng repository của TransactionEntity để lấy danh sách giao dịch
        // Sử dụng findAndCount để lấy cả dữ liệu và tổng số lượng giao dịch phù hợp với bộ lọc
    ...
    async getLinkPaymentVnpay(transaction: any) {
        let data = {
            ...
            "url_callback": "http://localhost:3000/cart"
        }
        return await this.serviceCore.getLinkPaymentVnpay(data)
    }

    async storeVnPay(transaction: any) {
        var tmnCode = '3RDGQAX3'; // Q58IN4ZE   3RDGQAX3
        var secretKey = 'PMSBQTYJIQLJILQTWHKAESOMMTXYHFHE'; // YRAXVPRGTUWRCVTWMWRDXUNQZREZVTCM PMSBQTYJIQLJILQTWHKAESOMMTXYHFHE
        var returnUrl = 'http://localhost:3000/cart'; 
        var vnpUrl = 'https://sandbox.vnpayment.vn/paymentv2/vpcpay.html'; // https://sandbox.vnpayment.vn/paymentv2/vpcpay.html
